...
// update methods | separate admin page for author data to finally make multi dropdown
    return (
        ...
        <div className="form-group mx-auto" style={{width:"75%"}}>
            <input type="text" className="form-control" id="AuthKeeper" name="authors" onChange={event => setAuthors(event.target.value)} value={authors} placeholder="Please enter Paper / Publication Title ... " required/>
            <small className="text-muted unselectable">Authors Temp</small>
        </div>
        <div className="form-group mx-auto" style={{width:"75%"}}>
            <input type="text" className="form-control" id="captionKeeper" name="venue" onChange={event => setVenue(event.target.value)} value={venue} placeholder="Venue / Conference / Journal as applicable ... " required/>
        ...
        <div className="output">
          ...
          { file && <ProgressBar file={file} setFile={setFile} title={title} authors={authors} links={links} venue={venue} setReset={reset}/> }
        </div>
      </form>
    )
}
